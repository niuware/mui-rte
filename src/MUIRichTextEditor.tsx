...
interface TMUIRichTextEditorStyles {
  ...
  inheritFontSize?: SystemStyleObject;
  editor?: SystemStyleObject;
  editorContainer?: SystemStyleObject;
  editorReadOnly?: SystemStyleObject;
  error?: SystemStyleObject;
  hidePlaceholder?: SystemStyleObject;
  placeHolder?: SystemStyleObject;
  linkPopover?: SystemStyleObject;
  linkTextField?: SystemStyleObject;
  anchorLink?: SystemStyleObject;
  toolbar?: SystemStyleObject;
  inlineToolbar?: SystemStyleObject;
}
...
> = (props, ref) => {
  const {
    ...
    readOnly,
    toolbar,
    inlineToolbarControls: inlineToolbarControlsProps,
    error,
    label,
    inheritFontSize,
    toolbarButtonSize,
  ...
  // @ts-ignore
  const muiRichTextEditorTheme = theme.mixins?.muiRichTextEditor as TMUIRichTextEditorStyles;

  const rootStyles = muiRichTextEditorTheme?.root ?? {};
  const containerStyles = muiRichTextEditorTheme?.container ?? {
  ...
  const inheritFontSizeStyles = muiRichTextEditorTheme?.inheritFontSize ?? {
    fontSize: 'inherit',
  };
  const editorStyles = muiRichTextEditorTheme?.editor ?? {};
  const editorContainerStyles = muiRichTextEditorTheme?.editorContainer ?? {
    margin: theme.spacing(1, 0, 0, 0),
    cursor: 'text',
    width: '100%',
    padding: theme.spacing(0, 0, 1, 0),
  };
  const editorReadOnlyStyles = muiRichTextEditorTheme?.editorReadOnly ?? {
    borderBottom: 'none',
  };
  const errorStyles = muiRichTextEditorTheme?.error ?? {
    borderBottom: '2px solid red',
  };
  const hidePlaceholderStyles = muiRichTextEditorTheme?.hidePlaceholder ?? {
    display: 'none',
  };
  const placeHolderStyles = muiRichTextEditorTheme?.placeHolder ?? {
    color: theme.palette.grey[600],
    position: 'absolute',
    outline: 'none',
  };
  const anchorLinkStyles = muiRichTextEditorTheme?.anchorLink ?? {};
  const toolbarSyles = muiRichTextEditorTheme?.toolbar ?? {};
  ...
  const handleChange = (state: EditorState) => setEditorState(state);
  ...
  const findAutocompleteStrategy = (chars: string): TAutocompleteStrategy | undefined => {
    ...
    if (acArray.length) return acArray[0];
    return undefined;
  };
  ...
  const confirmLink = (url?: string) => {
    ...
    // TODO this might cause potential bug
    const data = {
      url,
  ...
  const handleReturn = (_e: any, editorState: EditorState): DraftHandleValue =>
    isMaxLengthHandled(editorState, 1);
  ...
  const keyBindingFn = (e: React.KeyboardEvent<{}>): string | null => {
    if (hasCommandModifier(e) && keyCommands) {
  ...
  const inlineToolbarControls = inlineToolbarControlsProps || defaultInlineToolbarControls;
  ...
  if (!focus) {
    ...
    if (!contentState.hasText()) {
      placeholder = (
        <Box
          sx={[editorContainerStyles, placeHolderStyles, error ? errorStyles : {}]}
          tabIndex={0}
      ...
      styles = hidePlaceholderStyles;
    }
  }

  return (
    <Box id={`${editorId}-root`} sx={rootStyles}>
      <Box
        id={`${editorId}-container`}
        sx={[containerStyles, inheritFontSize ? inheritFontSizeStyles : {}]}
      >
        ...
        {placeholder}
        <Box id={`${editorId}-editor`} sx={editorStyles}>
          <Box
            id={`${editorId}-editor-container`}
            sx={[
              styles,
              editorContainerStyles,
              !editable ? editorReadOnlyStyles : {},
              error ? errorStyles : {},
            ]}
...
export default React.forwardRef(MUIRichTextEditor);
